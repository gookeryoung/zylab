"""flowchart.dsl DSL 模板测试：YAML 解析、校验、$ 参数代入与注册表接入."""
...
from zylab.flowchart.dsl import DslParam, DslTemplate, dsl_from_yaml, load_dsl
...
def test_substitute_value_list_recursion() -> None:
    """$ 引用在列表值内逐项替换."""
    from zylab.flowchart.dsl import substitute_refs
