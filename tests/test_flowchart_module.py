"""flowchart.module 模块类型系统测试：ParamSpec 收敛、ModuleSpec 查询、内置模块表完整性."""
...
class TestBuiltinModules:
    ...
    def test_targets_point_to_nodes_module(self) -> None:
        """执行目标均为 zylab.flowchart.nodes 下的函数（spawn 子进程可定位）."""
        for spec in BUILTIN_MODULES:
            assert spec.target.startswith("zylab.flowchart.nodes:"), spec.type_id
