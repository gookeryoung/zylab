"""gui.pages.flowchart_page 流程图页测试：模板实例化、参数联动、运行编排、节点交互."""
...
from zylab.flowchart import NodeRunEvent, NodeState
from zylab.gui.pages.flowchart_page import FlowchartPage, _FlowchartBridge


def _select_template(page: FlowchartPage, template_id: str) -> None:
    """按模板 id 切换模板下拉选择."""
...
def _capture_status(page: FlowchartPage) -> list[str]:
...
def _capture_run_status(page: FlowchartPage) -> list[tuple[str, str]]:
...
def test_preview_failure_marks_failed(qtbot, monkeypatch) -> None:
    ...
    monkeypatch.setattr("zylab.gui.pages.flowchart_page._resolve_target", _boom)
...
def test_save_template_as(qtbot, tmp_path) -> None:
    ...
    page = FlowchartPage(data_dir=tmp_path)
    qtbot.addWidget(page)
...
def test_project_save_and_load(qtbot, tmp_path) -> None:
    ...
    page2 = FlowchartPage(data_dir=tmp_path / "other")
