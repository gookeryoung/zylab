"""flowchart.registry 模板注册表测试：注册/查询/目录加载/插件 entry point."""
...
from zylab.flowchart import BUILTIN_TEMPLATES, Template, TemplateError, TemplateNotFoundError, TemplateRegistry
...
class TestLoadEntryPoints:
    ...
    def test_bad_factory_skipped(self) -> None:
        """非模板结果（元组）跳过不阻断."""
        plugins = self._plugins_with(
            PluginSpec(name="bad", kind=PluginKind.TEMPLATE, factory="zylab.flowchart:BUILTIN_TEMPLATES"),
