"""内置参数化计算：随包发布的预连节点图（assets/templates/<学科>/*）.

预制参数化计算以人类可读载体分发并按学科子目录归类：``*.json`` 为经典节点
节点图参数化计算，``*.yaml``/``*.yml`` 为 DSL 参数化计算（归一化为
:class:`~zylab.flowchart.dsl.DslTemplate`，与经典参数化计算同池注册）；用户可
参照其结构在 ``data_dir/templates/<学科>/`` 下自建参数化计算（注册表递归
加载），也可直接修改包内文件定制预制参数化计算的行为。
"""
...
def _load_assets_templates(directory: Path) -> tuple[Template, ...]:
    ...
    for path in paths:
        try:
            ...
            logger.warning("跳过非法预制参数化计算 %s: %s", path.name, exc)
    return tuple(templates)


#: 内置参数化计算表（模块导入时从包内资产加载：JSON 经典 + YAML DSL）
