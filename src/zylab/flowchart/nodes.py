...
from .errors import FlowchartError, ParamError
...
def compute_sweep(inputs: NodeInputs, params: NodeParams, report: ReportFn | None = None) -> dict[str, Any]:
    ...
    经 :func:`~zylab.flowchart.batch.run_workflow` 进程内执行整个子图
    ...
    for index, value in enumerate(values):
        ...
        if not outcome.succeeded:
            raise FlowchartError(f"参数扫描 {var}={value:.6g} 步失败: {outcome.first_error()}")
        for ref in collect:
