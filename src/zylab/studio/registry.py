"""参数化计算注册表：内置参数化计算随包注册，用户参数化计算从 ``data_dir/templates/*.json`` 目录加载.

第三方包可通过 entry point group ``zylab.template`` 注册插件参数化计算（工厂解析为
Template 实例或参数化计算字典），经 :meth:`TemplateRegistry.load_entry_points` 发现。
"""
...
class TemplateRegistry:
    ...
    def register(self, template: Template, *, replace: bool = False) -> None:
        """注册参数化计算；同 id 冲突默认抛 :class:`TemplateError`，``replace=True`` 时覆盖."""
        if template.id in self._templates and not replace:
            raise TemplateError(f"参数化计算 id 重复注册: {template.id!r}")
        self._templates[template.id] = template
        logger.debug("注册参数化计算: %s (%s)", template.id, template.name)

    def unregister(self, template_id: str) -> None:
        """注销参数化计算；未注册抛 :class:`TemplateNotFoundError`."""
        try:
            del self._templates[template_id]
        except KeyError:
            raise TemplateNotFoundError(f"参数化计算未注册: {template_id!r}") from None

    def get(self, template_id: str) -> Template:
        """按 id 取参数化计算；未注册抛 :class:`TemplateNotFoundError`."""
        try:
    ...
    def list(self, discipline: str | None = None) -> list[Template]:
        """列出参数化计算（按名称排序）；``discipline`` 非空时按学科过滤."""
    ...
    def load_dir(self, directory: Path) -> int:
        """加载目录下全部参数化计算（JSON 经典格式 + YAML DSL 格式），返回成功数量.

        顶层为历史平铺布局，子目录为按学科归类的现行布局；二者并存时全部加载。
        ``*.yaml``/``*.yml`` 走 DSL 解析（归一化为 :class:`DslTemplate`，
        与经典参数化计算同池注册）；单个文件非法仅记录告警并跳过。
        """
        directory = Path(directory)
        if not directory.is_dir():
            logger.warning("参数化计算目录不存在，跳过加载: %s", directory)
            return 0
        ...
        for path in paths:
            try:
                ...
                logger.warning("跳过非法参数化计算文件 %s: %s", path.name, exc)
                continue
            count += 1
        return count

    def load_entry_points(self, plugins: PluginRegistry | None = None) -> int:
        """发现并注册 entry point group ``zylab.template`` 声明的插件参数化计算，返回注册数量.

        工厂解析结果可为 :class:`Template` 实例或参数化计算字典（经 from_dict 校验）；
        单个插件失败仅记录告警并跳过。
        ...
        for spec in plugins.list(kind=PluginKind.TEMPLATE):
            try:
                ...
                logger.warning("跳过非法插件参数化计算 %s: %s", spec.name, exc)
                continue
            count += 1
        return count
...
def _expect_mapping(obj: object, name: str) -> Mapping[str, Any]:
    ...
    if not isinstance(obj, Mapping):
        raise TemplateError(f"插件参数化计算 {name!r} 工厂应解析为 Template 或字典，得到 {type(obj).__name__}")
    return obj
