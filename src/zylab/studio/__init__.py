"""zylab.studio - 参数化计算配置化多学科分析工作台内核（Qt-free）.
...
- :mod:`zylab.studio.template`：参数化计算定义与 JSON 加载/校验；
- :mod:`zylab.studio.registry`：参数化计算注册表（内置 + 用户目录）；
- :mod:`zylab.studio.builtin`：内置参数化计算表；
...
- :mod:`zylab.studio.dsl`：DSL 参数化计算（YAML 声明式参数化计算解析/校验/参数绑定）；
