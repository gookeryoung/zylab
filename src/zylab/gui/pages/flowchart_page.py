"""流程图页：Workbench 风格三区整合（工具栏 + 系统画布/结果 + 参数表单 + 底部状态栏）.
...
class _FlowchartBridge(QObject):
...
class FlowchartPage(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None, data_dir: Path | None = None) -> None:
        """初始化流程图页：参数化计算注册表（内置 + 用户目录 + 插件）+ Workbench 布局."""
    ...
    def _build_ui(self) -> None:
        ...
        splitter.setStretchFactor(0, 1)  # 中央画布拉伸
        splitter.setStretchFactor(1, 0)  # 参数表单固定宽度
